...
rm -rf "$WORKSPACE"
mkdir -p "$WORKSPACE/src/orders" "$WORKSPACE/src/payments" "$WORKSPACE/src/shipping"

# Duplicated validation logic in 3 modules
cat > "$WORKSPACE/src/orders/validate.ts" << 'EOF'
...
cat > "$WORKSPACE/src/payments/validate.ts" << 'EOF'
...
cat > "$WORKSPACE/src/shipping/validate.ts" << 'EOF'
...
cat > "$WORKSPACE/src/orders/format.ts" << 'EOF'
...
cat > "$WORKSPACE/src/payments/format.ts" << 'EOF'
...
cat > "$WORKSPACE/src/shipping/format.ts" << 'EOF'
...
grep -rh "email.*includes.*@" "$WORKSPACE/src/" | wc -l > "$WORKSPACE/.dup-before-email"
grep -rh "getFullYear.*getMonth.*getDate" "$WORKSPACE/src/" | wc -l > "$WORKSPACE/.dup-before-date"
