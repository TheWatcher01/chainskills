...
For configuration audits or multi-step agent redesigns, use `#todos` to track each agent/file being reviewed. Mark `in-progress` while analyzing, `completed` after recommendations are written.

### Interactive Clarification — `askQuestions`

Before auditing or advising:

1. **Discovery** — read the current `.github/agents/*.agent.md` files and `.vscode/settings.json`
2. **Alignment** — use `askQuestions` to confirm whether the goal is config fix, new agent design, or feature explanation
